...
class OrdemServicoView(View):
    ...
    def post(self, request):
        if request.method == "POST":
            ...
            dados_comprador = DadosComprador(nome_comprador=form["nome_comprador"], cpf_cnpj =form["id_comprador"], rg_ie=form["sub_id_comprador"], data_nascimento_comprador=form["data_nascimento-comprador"],
            ...
            dados_compra = DadosCompra(nome_loja =form["loja"], num_nf=form["nf"], data_compra=form["data_compra"],
                                         cod_produto=form["cod_produto"], valor=form["valor"])
            dados_compra.save()

            dados_equipamento = DadosEquipamento(nome_equipamento=form["equipamento"], marca =form["marca"], lacrado=form["estado_lacre"], modelo=form["modelo"],
